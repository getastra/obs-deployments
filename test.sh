...
function runAstraSecretScan() {
    ...
    if [ -z "$ASTRA_SECRET_SCAN_CONFIG_PATH" ]; then
        ...
        error_output=$("$BIN_PATH/$BINARY_NAME" dir "$ASTRA_GIT_ROOT" --report-format=json --no-banner --max-target-megabytes=1 --exit-code=0 --redact=50 --max-decode-depth=1 --report-path=astra-secret-scan-report.json)
        exit_code=$?
        ...
        echo "Scanning git repository at: $ASTRA_GIT_ROOT"
        error_output=$("$BIN_PATH/$BINARY_NAME" dir "$ASTRA_GIT_ROOT" --config="$ASTRA_SECRET_SCAN_CONFIG_PATH" --report-format=json --no-banner --max-target-megabytes=1 --exit-code=0 --redact=50 --max-decode-depth=1 --report-path=astra-secret-scan-report.json)
        exit_code=$?
        if [ $exit_code -ne 0 ]; then
