...
function runAstraSecretScan() {
    ...
    if [ -z "$ASTRA_SECRET_SCAN_CONFIG_PATH" ]; then
        ...
        $BIN_PATH/$BINARY_NAME dir "$ASTRA_GIT_ROOT" --report-format=json --no-banner --max-target-megabytes=1 --exit-code=0 --max-decode-depth=1 --report-path=astra-secret-scan-report.json
